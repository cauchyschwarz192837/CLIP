...
class DatasetBase:
    ...
    @property
    def classnames(self): # clip_weights = clip_classifier(dataset.classnames, dataset.template, clip_model)
    ...
    def get_lab2cname(self, data_source): # self._lab2cname, self._classnames = self.get_lab2cname(train_x)
    ...
    def generate_fewshot_dataset(
        ...
        """Generate a few-shot TOP-K dataset (typically for the training set).

        This function is useful when one wants to evaluate a model
        in a few-shot learning setting where each class only contains
        a few number of images (BUT THIS MUST BE THE TOP K MOST SIMILAR IMAGES)
        ...
        for data_source in data_sources:
            tracker = self.split_dataset_by_label(data_source) # list of Datums
            dataset = []
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            if isinstance(self.transform, (list, tuple)):
                for i, tfm in enumerate(self.transform): # go through the series of transformations
                    img = self._transform_image(tfm, img0)
                    keyname = 'img'
                    if (i + 1) > 1:
                        keyname += str(i + 1)
                    output[keyname] = img # add entries at different stages to output # can extract features from here later
            else:
